...
async function cometChatRequest<T>(path: string, options: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`CometChat API ${path} failed (${response.status}): ${raw}`);
  }
...
export async function createCometChatAuthToken(uid: string): Promise<string> {
  ...
  const firstData = Array.isArray(payload.data) ? payload.data[0] : payload.data;
  const token = firstData?.authToken ?? firstData?.token ?? payload.authToken ?? payload.token;
  if (!token) {
    throw new Error(`CometChat auth token not returned for uid=${uid}. Payload: ${JSON.stringify(payload)}`);
  }

  return token;
}

export async function addFriendsBidirectional(uidA: string, uidB: string): Promise<void> {
  await Promise.all([
