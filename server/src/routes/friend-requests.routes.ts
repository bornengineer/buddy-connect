...
friendRequestRouter.get("/stream", async (req, res) => {
...
friendRequestRouter.get("/incoming", async (req, res) => {
  const currentUserId = req.auth!.userId;
...
friendRequestRouter.patch("/:requestId", async (req, res) => {
  ...
  if (parse.data.action === "reject") {
    ...
    pushToUser(request.sender_id, "friend_request_rejected", { requestId: id });
